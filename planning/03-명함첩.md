...
| createdAt | datetime | 요청일시 |
| respondedAt | datetime | 응답일시 (nullable) |
...
| Method | Endpoint | 설명 |
|--------|----------|------|
| POST | /api/cardbooks/:id/exchange | 교환 요청 (내 명함 전송) |
| GET | /api/cardbooks/exchanges/received | 받은 교환 요청 목록 |
| GET | /api/cardbooks/exchanges/received/count | 대기중인 요청 수 (배지용) |
| POST | /api/cardbooks/exchanges/:id/accept | 수락 |
| POST | /api/cardbooks/exchanges/:id/reject | 거절 |
...
### GET /api/cardbooks/exchanges/received
받은 교환 요청 목록 조회
```json
...
### GET /api/cardbooks/exchanges/received/count
대기중인 교환 요청 수 (배지 표시용)
```json
// Response
{
...
### POST /api/cardbooks/exchanges/:id/accept
교환 요청 수락 - 발신자의 명함이 자동으로 내 명함첩에 저장됨
```json
// Response
{
  "id": "uuid",
...
// 서버 동작:
// 1. card_exchanges 상태를 ACCEPTED로 변경
// 2. 발신자의 명함을 내 명함첩에 저장 (card_books 테이블)
// 3. 발신자에게 수락 알림 발송
```

### POST /api/cardbooks/exchanges/:id/reject
교환 요청 거절
```json
// Response
{
