...
| Method | Endpoint | 설명 |
|--------|----------|------|
| POST | /api/card-exchanges | 교환 요청 생성 |
| GET | /api/card-exchanges/received | 받은 교환 요청 목록 |
| GET | /api/card-exchanges/received/count | 대기중인 요청 수 (배지용) |
...
### POST /api/card-exchanges
교환 요청 생성 - 명함 저장 후 "내 명함 전송" 선택 시 호출
```json
// Request
{
...
### GET /api/card-exchanges/received/count
대기중인 교환 요청 수 (배지 표시용)
```json
// Response
{
...
### POST /api/card-exchanges/:id/reject
교환 요청 거절
```json
// Response
{
